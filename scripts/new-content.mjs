// Скаффолдер контента Armenia Guidebook (ROADMAP #20b).
// Сайт одноязычный (EN-first) — генерит ОДНУ языковую версию (en) скелета с
// корректным по content.config.ts frontmatter + папку фото public/images/<slug>/ + .gitkeep.
//
// Использование:
//   node scripts/new-content.mjs <type> <slug> [--title "Title"] [--city Yerevan]
...
const DOD = {
  article: [
    'en-версия на месте (сайт одноязычный);',
    ...
    'title ≤60, description ≤155, hreflang self en + x-default, schema.org проходит Rich Results Test;',
