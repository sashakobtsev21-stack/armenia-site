...
/**
 * Контент-модель Armenia Guidebook (SPEC §11; форк движка сети путеводителей).
 *
 ...
 *
 * Коллекции на старте ПУСТЫЕ — контент не создаём. Сборка обязана проходить
 ...
 * хранит флаг.
 *
 * Язык: EN-first. На старте LANGS=['en'] (англоязычные форки сети). ru-движок
 * донора оставлен ДОРМАНТНЫМ (i18n/ru.ts.dormant + ru-способные компоненты),
 * ru-контента нет. ru-кластер релокации — фаст-фоллоу позже флипом LANGS=['en','ru'].
 */
 ...
 * карточка по-прежнему показывает полную строку `cuisine`. Под Армению:
 * армянская кухня + кавказская + хоровац (гриль) + вино (Арени/Вайоц Дзор) +
 * бренди (Ереван). Армения не имеет выхода к морю — без `seafood`.
 */
const CUISINE_KEYS = [
  'armenian',
  'caucasian',
 ...
 * лейблы в i18n (`attractionTypes`); набор финализирован: 7 типов. Под Армению
 * главный кластер — монастыри (Хор Вирап, Татев, Гегард, Нораванк).
 */
const ATTRACTION_TYPES = [
  'mountains-nature', // горы / перевалы / природа (Арагац, Дилижан)
  'lakes-canyons', // озёра / каньоны (Севан, красные скалы Нораванка)
  'caves', // пещеры (пещера Арени-1)
  'churches-monasteries', // храмы / монастыри (главный кластер)
  'fortresses-castles', // крепости (Амберд)
  'spa-resorts', // курорты / термы (Джермук, Цахкадзор, Дилижан)
  'museums-urban', // музеи / городские объекты (Каскад, Матенадаран)
] as const;

/**
 * Регион Армении (§7) — для фильтра каталога достопримечательностей.
 * Опционально (используют только достопримечательности). Слаги ↔ лейблы
 * в i18n (`regions`). Армения = 10 марзов + Ереван; для каталога сгруппированы
 * в 3 макрорегиона (макро-группировка — не плодить тонкие фильтры, план §1):
 *   central  — Ереван + Арарат (Хор Вирап), Армавир (Эчмиадзин), Котайк
 *              (Гарни/Гегард), Арагацотн (Арагац/Амберд);
 *   southern — Вайоц Дзор (Арени/Нораванк/Джермук), Сюник (Татев/Горис);
 *   northern — Гегаркуник (Севан), Тавуш (Дилижан), Лори (Ахпат/Санаин), Ширак (Гюмри).
 */
const REGIONS = ['central', 'southern', 'northern'] as const;
 ...
 * для живущих в Армении (риелторы, клининг и т.п.). Слаги ↔ en лейблы в i18n
 * (`serviceRubrics`).
 */
...
const articleBase = z.object({
  ...
  /**
   * Регион Армении (§7) — фильтр/чип каталога достопримечательностей.
   ...
   * местах получают блок AccessFrom с расстоянием/временем из Еревана
   * (столица + аэропорт Звартноц), Гюмри (2-й город) и Дилижана (вход на
   * север). Все поля внутри опциональны — указываем только то, что известно
 ...
 * живущих в Армении (риелторы, клининг, переезд и т.п.). Платное размещение —
