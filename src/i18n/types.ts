 ...
 */
export const EDA_CITY_KEYS = ['yerevan', 'gyumri', 'dilijan'] as const;
export type EdaCityKey = (typeof EDA_CITY_KEYS)[number];

/**
 * Слаги регионов Армении (§7) — совпадают с enum REGIONS в content.config.ts
 * (поле `region`). Лейблы — в словарях (`regions`). 3 макрорегиона (10 марзов +
 * Ереван сгруппированы): central / southern / northern.
 */
export const REGION_SLUGS = ['central', 'southern', 'northern'] as const;
...
export interface UIDictionary {
  ...
  /** Блок «Сейчас в Армении» на главной (§8.4): погода + курс драма. */
  liveData: {
    heading: string;
    ...
    sea: string;
    fx: string;
    cities: { yerevan: string; gyumri: string; dilijan: string; sevan: string };
    updated: string;
    source: string;
  };
   ...
   * точек въезда. Лейблы городов-истоков по слугам yerevan/gyumri/dilijan.
   */
  access: {
   ...
   * InsuranceHub). Регуляторная тема: действующие правила въезда в Армению +
  ...
  /** Лейблы регионов Армении (§7) по слугам REGION_SLUGS. */
   ...
   * Армении. Шаблон-директория (ServicesDirectory) с фильтром по рубрике;
